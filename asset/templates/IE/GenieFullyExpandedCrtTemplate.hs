concrete {crt_grammar_name} of {abs_grammar_name} = {{
  ...
  lincat
    Text , BOG , EOG , Triplet , Triplets , Subject , Relation , Object , TripletEndingMarker , SubjectMarker , RelationMarker , objectMarker , Rel , Entity = Str;
  lin
    Start x y z = x ++ y ++ z;
    Repeat x y = x ++ y;
    Empty_triplet = [];
    Empty_triplets = [];
    Derive_triplet x y z w = x ++ y ++ z ++ w ;
    Derive_triplets x y z w = x ++ y ++ z ++ w ;
