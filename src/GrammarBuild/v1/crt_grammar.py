...
class GenieCrtGrammarBuilder(TemplateTokenGrammarBuilder):
    template = os.path.join(TEMPLATE_DIR, "v1", "GenieFullyExpandedCrtTemplate.txt")
    grammar_prefix = ""
    ...
    def get_entity_or_rel_decoding_lin(self, entity: str=None, rel:str=None, tokenizer=None) -> str:
        ...
        input_vars: List[str] = [f"x_{i}" for i in range(len(token_ids))]
        input_string = " ".join(input_vars)
        output_string = " ++ ".join(input_vars)
        return f"{func_name} {input_string} = {output_string};"

    def get_token_decoding_lin(self, token_id: int, tokenizer):
        """Tok_28917_lin =  "a";"""
        func_name: str = self.get_tokenization_func_name(token_id=token_id)
        token_id_with_quotes = f'"{token_id}"'
        return f"{func_name} = {token_id_with_quotes};"
