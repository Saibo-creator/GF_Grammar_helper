...
class ELotfeCrtGrammarBuilder(ELCrtGrammarBuilder, ABC):
    template = os.path.join(TEMPLATE_DIR, "EL", "EL-OTF-efficient-CrtTemplate.hs")
    grammar_prefix = ""
    ...
    def build(self, base_grammar_name: str, entities_or_path: Union[List[str], str, List[List[str]]], crt_grammar_name=None, mentions: List[str] = None, contexts: List[str] = None, **kwargs) -> Grammar:
        assert mentions is not None, "mention should not be None"
...
if __name__ == '__main__':
    grammar_builder = ELotfeCrtGrammarBuilder(tokenizer_or_path="/Users/saibo/Research/llama_hf/7B", literal=True)
    grammar = grammar_builder.build(base_grammar_name="EL-efficient", mentions=["mention0","mention1","mention2"],contexts=["context0","context1","context2", "context3"],entities_or_path=[["ent0", "ent1", "ent2"]])
    grammar.save("./EL-efficient")
