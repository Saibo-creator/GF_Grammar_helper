...
def get_prod_name_for_ie(
    entity: str = None, rel: str = None, token_id: int = None
) -> str:
    return get_tokenization_func_name(entity=entity, rel=rel, token_id=token_id)


def get_tokenization_func_name(
    entity: str = None, rel: str = None, token_id: int = None
) -> str:
    # TODO: change name to get_prod_name_for_ie
...
class TemplateTokenGrammarBuilder:
    ...
    def __init__(self, tokenizer_or_path: str, literal=False):
        ...
        ), "tokenizer_or_path must be a string, passing a tokenizer object is not supported yet"
        self.tokenizer = AutoTokenizer.from_pretrained(
            tokenizer_or_path, use_fast=False
        )
    ...
    def post_process_token_ids(
        self, token_ids: List[int], rm_bos=True, rm_eos=False, pseudo_prefix=False
    ) -> List[Union[int, str]]:
        "remove_bos=True, remove_eos=False"
        # N.B. case where token_ids is empty
        if (
            len(token_ids) != 0
            and token_ids[0] == self.tokenizer.bos_token_id
            and rm_bos
        ):
        ...
        pseudo_prefix_token_id = (
            self.tokenizer.encode(self.PSEUDO_PREFIX, add_special_tokens=False)[0]
            if pseudo_prefix
            else None
        )

        if (
            pseudo_prefix_token_id is not None
            and token_ids[0] == pseudo_prefix_token_id
        ):
            token_ids = token_ids[1:]
    ...
    def get_grammar_name(self, base_grammar_name: str, crt=False, idx=None) -> str:
        ...
        return (
            f"{base_grammar_name}{self.grammar_suffix}"
            if not crt
            else f"{base_grammar_name}{self.grammar_suffix}_Crt"
        )
    ...
    def get_materialization_rule(
        ...
        if type(entity) != str:
            print(
                f"entity {entity} is not a string! It is {type(entity)}. Converting to string..."
            )
            entity = str(entity)

        tokens_concat = self.get_entity_tokens(
            entity,
            rm_bos=True,
            rm_eos=True,
