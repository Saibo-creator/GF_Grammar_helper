...
from src.legacy_GrammarBuild.CP.indep.indep_abs_grammar import (
    CP_IndepPtbAbsGrammarBuilder,
)
...
class CP_DepPtbReAbsGrammarBuilder(CP_IndepPtbAbsGrammarBuilder):
    ...
    template = os.path.join(
        TEMPLATE_DIR, "CP", "Dep", "ptb-re", "CP-Dep-PTB-AbsTemplate.hs"
    )
    grammar_prefix = ""
    ...
    def get_Rule8_1D_single(self, i: int, n_words: int) -> str:
        ...
        return (
            f"Rule8_E_{n_words}_{i} : Right -> E_{n_words}_{i-1} -> E_{n_words}_{i} ;"
        )
    ...
    def get_RuleW_1D(self, n_words: int) -> str:
        return self.join_statements_multi_line(
            statements=[self.get_RuleW_1D_single(i) for i in range(n_words)]
        )
    ...
    def get_Derive_FullPhraseLevelTags(self) -> str:
        return self.join_statements_multi_line(
            statements=[
                self.get_Derive_FullPhraseLevelTags_single(tag.replace("-", "_"))
    ...
    def get_Derive_WordLevelTags(self) -> str:
        return self.join_statements_multi_line(
            statements=[
                self.get_Derive_WordLevelTags_single(
                    tag.replace("-", "_").replace("$", "Dollar")
                )
...
if __name__ == "__main__":
    grammar = CP_DepPtbReAbsGrammarBuilder(
