...
from src.legacy_GrammarBuild.CP.indep.indep_crt_grammar import (
    CP_IndepPtbCrtGrammarBuilder,
)


class CP_DepPtbReCrtGrammarBuilder(CP_IndepPtbCrtGrammarBuilder):
    template = os.path.join(
        TEMPLATE_DIR, "CP", "Dep", "ptb-re", "CP-Dep-PTB-CrtTemplate.hs"
    )
    ...
    def build(self, base_grammar_name: str, **kwargs) -> Grammar:
        ...
        formatted_grammar_plain_text: str = grammar.format(
            ...
            Left_Paren=self.get_entity_tokens(
                entity=self.Left_Paren, rm_bos=True, rm_eos=True
            ),
            Right_Paren=self.get_entity_tokens(
                entity=self.Right_Paren, rm_bos=True, rm_eos=True
            ),
            # Rule0_0D=Rule0_0D,
            Rule1_2D=Rule1_2D,
            Rule2_2D=Rule2_2D,
    ...
    def get_Rule1_2D(self, n_words: int) -> str:
        return self.join_statements_multi_line(
            statements=[
                self.get_Rule1_2D_single(i, j)
                for i in range(n_words)
    ...
    def get_Rule5_1D(self, n_words: int) -> str:
        return self.join_statements_multi_line(
            statements=[
                self.get_Rule5_1D_single(i, n_words) for i in range(n_words + 1)
            ]
        )
    ...
    def get_Rule8_1D(self, n_words: int) -> str:
        return self.join_statements_multi_line(
            statements=[
                self.get_Rule8_1D_single(i, n_words) for i in range(1, n_words + 1)
            ]
        )
    ...
    def get_RuleW_1D(self, words: List[str]) -> str:
        return self.join_statements_multi_line(
            statements=[
                self.get_RuleW_1D_single(i, entity=words[i]) for i in range(len(words))
            ]
        )
    ...
    def get_RuleW_1D_single(self, i: int, entity: str) -> str:
        ...
        rule = self.get_materialization_rule(
            rule_name=f"Materialize_W{i}", entity=entity
        )
        return rule
    ...
    def get_Derive_FullPhraseLevelTags(
        self,
    ) -> str:
        return self.join_statements_multi_line(
            statements=[
                self.get_materialization_rule(
                    rule_name=f'Derive_FullPhraseLevelTag_{tag.replace("-","_")}',
                    entity=tag,
                )
    ...
    def get_Derive_WordLevelTags(
        self,
    ) -> str:
        return self.join_statements_multi_line(
            statements=[
                self.get_materialization_rule(
                    rule_name=f'Derive_WordLevelTag_{tag.replace("-","_").replace("$","Dollar")}',
                    entity=tag,
                )
...
if __name__ == "__main__":
    ...
    grammar_builder = CP_DepPtbReCrtGrammarBuilder(
        tokenizer_or_path="/home/saibo/Research/llama_models/7B", literal=True
    )
    grammar = grammar_builder.build(
        base_grammar_name="CP_PTB_RE",
        crt_grammar_name="CP_PTB_RE_Crt",
        words=["I", "love", "you"],
    )
