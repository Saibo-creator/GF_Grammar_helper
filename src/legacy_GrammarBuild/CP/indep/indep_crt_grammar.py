...
from src.legacy_GrammarBuild.CP.const import (
    PHRASE_LEVEL_TAGS,
    FUNCTIONAL_TAGS,
    WORD_LEVEL_TAGS,
    NUM_TAGS,
...
class CP_IndepPtbCrtGrammarBuilder(TemplateTokenGrammarBuilder, ABC):
    template = os.path.join(
        TEMPLATE_DIR, "CP", "InDep", "ptb", "CP-Indep-PTB-CrtTemplate.hs"
    )
    ...
    def build(self, base_grammar_name: str, **kwargs) -> Grammar:
        ...
        formatted_grammar_plain_text: str = grammar.format(
            ...
            Hyphen=self.get_entity_tokens(
                entity=self.Hyphen, rm_bos=True, rm_eos=True
            ),  # , pseudo_prefix="Ж"
            ...
            Input_Word=self.get_entity_tokens(
                entity=self.Input_Word, rm_bos=True, rm_eos=True
            ),
    ...
    def get_PhraseLevelTag_LinearizationRule(self, tag: str) -> str:
        ...
        materialization_rule: str = self.get_materialization_rule(
            rule_name=rule_name, entity=tag
        )
    ...
    def get_HyphenFunctionTag_LinearizationRule(self, tag: str) -> str:
        ...
        materialization_rule: str = self.get_materialization_rule(
            rule_name=rule_name, entity=hyphen_tag, pseudo_prefix=True
        )
        return materialization_rule
...
if __name__ == "__main__":
    ...
    grammar = CP_IndepPtbCrtGrammarBuilder(
        tokenizer_or_path="/Users/saibo/Research/llama_hf/7B", literal=False
    ).build(base_grammar_name="CP-PTB", crt_grammar_name="CP-PTB-Crt")
