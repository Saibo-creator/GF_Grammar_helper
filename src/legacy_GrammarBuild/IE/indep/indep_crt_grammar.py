...
from src.legacy_GrammarBuild.grammar_builder import TemplateTokenGrammarBuilder
...
class IE_CrtGrammarBuilder(TemplateTokenGrammarBuilder, ABC):
    ...
    def build(self, base_grammar_name: str, **kwargs) -> Grammar:
        ...
        relations_or_path: Union[List[str], str, List[List[str]]] = kwargs[
            "relations_or_path"
        ]
        ...
        relations = (
            self.read_jsonl(relations_or_path)
            if isinstance(relations_or_path, str)
            else relations_or_path
        )
        formatted_grammar_plain_text: str = grammar.format(
            ...
            SubjectMarker_tokens=self.get_entity_tokens(
                self.SubjectMarker, rm_eos=True
            ),
            RelationMarker_tokens=self.get_entity_tokens(
                self.RelationMarker, rm_eos=True
            ),
            objectMarker_tokens=self.get_entity_tokens(self.objectMarker, rm_eos=True),
            TripletEndingMarker_tokens=self.get_entity_tokens(
                self.TripletEndingMarker, rm_eos=True
            ),
            Materialize_Entities=self.batch_get_decoding_linearization_rules(
                entities=entities, rm_eos=True, rm_bos=True
            ),
            rel_lin_str=self.batch_get_decoding_linearization_rules(
                relations=relations, rm_eos=True, rm_bos=True
            ),
        )
        return Grammar(formatted_grammar_plain_text, name=crt_grammar_name)

    def batch_get_decoding_linearization_rules(
        self,
        ...
        if entities:
            statements = [
                self.get_entity_or_rel_decoding_linearization_rule(
                    entity=entity, rm_bos=rm_bos, rm_eos=rm_eos
                )
                for entity in tqdm(entities, desc="get linearization for entities")
            ]
        elif relations:
            statements = [
                self.get_entity_or_rel_decoding_linearization_rule(
                    rel=rel, rm_bos=rm_bos, rm_eos=rm_eos
                )
                for rel in tqdm(relations, desc="get linearization for relations")
            ]
        else:
            raise ValueError("No input_ids provided!")
        return self.join_statements_multi_line(statements)

    def get_entity_or_rel_decoding_linearization_rule(
        self, entity: str = None, rel: str = None, rm_bos=True, rm_eos=False
    ) -> str:
        """Germany = "ger"++"many"; France = "fra"++"nce"; UK = "u"++"k"; US = "u"++"s";"""
        if entity:
            func_name: str = get_tokenization_func_name(entity=entity)
        elif rel:
            func_name: str = get_tokenization_func_name(rel=rel)
            entity = rel
        else:
            raise ValueError("No input_ids provided!")
        token_ids: List[int] = self.tokenizer.encode(entity)
        processed_token_ids: List[Union[int, str]] = self.post_process_token_ids(
            token_ids, rm_bos=rm_bos, rm_eos=rm_eos
        )
        token_id_in_quotes: List[str] = [
            f'"{token_id}"' for token_id in processed_token_ids
        ]
...
class IE_IndepFullyExpandedCrtGrammarBuilder(IE_CrtGrammarBuilder):
    ...
    template = os.path.join(
        TEMPLATE_DIR, "IE", "InDep", "FE", "IE-Indep-FullyExpanded-CrtTemplate.hs"
    )
...
class IE_IndepSubjectCollapsedCrtGrammarBuilder(IE_CrtGrammarBuilder):
    ...
    template = os.path.join(
        TEMPLATE_DIR, "IE", "InDep", "SC", "IE-Indep-SubjectCollapsed-CrtTemplate.hs"
    )
