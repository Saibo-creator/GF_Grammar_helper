...
class IE_AbsGrammarBuilder(TemplateTokenGrammarBuilder):
    ...
    def build(self, base_grammar_name: str, **kwargs) -> Grammar:
        ...
        formatted_grammar_plain_text: str = grammar.format(
            ...
            Materialize_Relations=self.add_relations_derivative_rules(
                relations=relations
            ),
        )
        ...
        return Grammar(
            formatted_grammar_plain_text, name=abs_grammar_name, meta=grammar_meta
        )

    def assign_entities_ids(self, entities: List[str]) -> Dict[str, str]:
        return {
            get_tokenization_func_name(entity=entity): entity for entity in entities
        }

    def assign_relations_ids(self, relations: List[str]) -> Dict[str, str]:
        return {
            get_tokenization_func_name(rel=relation): relation for relation in relations
        }
    ...
    def add_entities_derivative_rules(self, entities: List[str]) -> str:
        ...
        entities_ids = [
            entity_key if i == len(entities_id_map) - 1 else entity_key + ","
            for i, entity_key in enumerate(entities_id_map.keys())
        ]
    ...
    def add_relations_derivative_rules(self, relations: List[str]) -> str:
        relations_id_map: Dict[str, str] = self.assign_relations_ids(
            relations=relations
        )
        relations_ids = [
            relation_key if i == len(relations_id_map) - 1 else relation_key + ","
            for i, relation_key in enumerate(relations_id_map.keys())
        ]
...
class IE_IndepFullyExpandedAbsGrammarBuilder(IE_AbsGrammarBuilder):
    template = os.path.join(
        TEMPLATE_DIR, "IE", "InDep", "FE", "IE-Indep-FullyExpanded-AbsTemplate.hs"
    )
...
class IE_IndepSubjectCollapsedAbsGrammarBuilder(IE_AbsGrammarBuilder):
    template = os.path.join(
        TEMPLATE_DIR, "IE", "InDep", "SC", "IE-Indep-SubjectCollapsed-AbsTemplate.hs"
    )
