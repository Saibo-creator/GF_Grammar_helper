...
class ED_DepSentenceCrtGrammarBuilder(ED_IndepMinimalCrtGrammarBuilder, ABC):
    ...
    template = os.path.join(
        TEMPLATE_DIR, "ED", "Dep", "sentence", "ED-Dep-sentence-CrtTemplate.hs"
    )
    grammar_prefix = ""
    ...
    def build(self, base_grammar_name: str, **kwargs) -> Grammar:
        ...
        left_context: str = kwargs["left_context"]
        right_context: str = kwargs["right_context"]
        ...
        formatted_grammar_plain_text: str = grammar.format(
            ...
            RightContext_tokens=self.get_entity_tokens(
                entity=right_context, rm_eos=True
            ),
            StartMarker_tokens=self.get_entity_tokens(
