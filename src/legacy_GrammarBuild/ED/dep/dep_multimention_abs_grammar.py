...
class ED_DepMultiMentionAbsGrammarBuilder(ED_IndepMinimalAbsGrammarBuilder):
    ...
    template = os.path.join(
        ...
        "Dep",
        "multi-mention",
        "ED-Dep-multi-mention-AbsTemplate.hs",
    )
    grammar_prefix = ""
    ...
    NUMERATED_RIGHT_CONTEXT_MATERIALISATION_RULE_TEMPLATE = (
        "Materialise_RightContext{i}: RightContext{i} ;"
    )
    NUMERATED_MENTION_MATERIALISATION_RULE_TEMPLATE = (
        "Materialise_Mention{i}: Mention{i} ;"
    )
    ...
    def build(self, base_grammar_name: str, **kwargs) -> Grammar:
        ...
        NumeratedMentionMaterialisationRules = (
            self.get_NumeratedMentionMaterialisationRules(num_mentions=num_mentions)
        )
        NumeratedEntitiesMaterialisationRules = (
            self.get_NumeratedEntitiesMaterialisationRules(entities=entities_or_path)
        )
    ...
    def get_NumeratedRightContextMaterialisationRules(self, num_mentions: int):
        RightContextMaterialisationRules = [
            self.NUMERATED_RIGHT_CONTEXT_MATERIALISATION_RULE_TEMPLATE.format(i=i)
            for i in range(num_mentions)
        ]
    ...
    def get_NumeratedMentionMaterialisationRules(self, num_mentions: int):
        MentionMaterialisationRules = [
            self.NUMERATED_MENTION_MATERIALISATION_RULE_TEMPLATE.format(i=i)
            for i in range(num_mentions)
        ]
    ...
    def get_NumeratedEntitiesMaterialisationRules(self, entities: List[List[str]]):
        NumeratedEntitiesMaterialisationRules = [
            self.add_entities_derivative_rules(entities[i], i)
...
if __name__ == "__main__":
    grammar = ED_DepMultiMentionAbsGrammarBuilder(
        tokenizer_or_path="/Users/saibo/Research/llama_hf/7B", literal=True
    ).build(
        base_grammar_name="ED-OTFE",
        num_mentions=2,
        entities_or_path=[["Q1", "Q2", "Q3", "Q4"], ["Q5", "Q6", "Q7", "Q8"]],
    )
