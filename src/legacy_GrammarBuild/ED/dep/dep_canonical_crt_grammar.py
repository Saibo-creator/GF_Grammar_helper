...
class ED_DepCanonicalCrtGrammarBuilder(ED_IndepMinimalCrtGrammarBuilder, ABC):

    template = os.path.join(
        TEMPLATE_DIR, "ED", "Dep", "canonical", "ED-Dep-Canonical-CrtTemplate.hs"
    )
    grammar_prefix = ""

    Open_bracket_marker = "["
    Close_bracket_marker = "]"
    Canonical_phrase_prompt = ": Canonical Form"
    ...
    def build(self, base_grammar_name: str, **kwargs) -> Grammar:
        ...
        formatted_grammar_plain_text: str = grammar.format(
            ...
            canonical_phrase_tokens=self.get_entity_tokens(
                self.Canonical_phrase_prompt, rm_eos=True
            ),
