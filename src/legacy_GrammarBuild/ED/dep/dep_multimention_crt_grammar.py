...
from src.legacy_GrammarBuild.base_grammar import Grammar
...
class ED_DepMultiMentionCrtGrammarBuilder(ED_IndepMinimalCrtGrammarBuilder, ABC):
    template = os.path.join(
        ...
        "Dep",
        "multi-mention",
        "ED-Dep-multi-mention-CrtTemplate.hs",
    )
    grammar_prefix = ""
    ...
    NUMERATED_RIGHT_CONTEXT_MATERIALISATION_RULE_TEMPLATE = (
        "Materialise_RightContext{i}: {tokens} ;"
    )
    NUMERATED_MENTION_MATERIALISATION_RULE_TEMPLATE = (
        "Materialise_Mention{i}: {tokens} ;"
    )
    ...
    def build(self, base_grammar_name: str, **kwargs) -> Grammar:
        ...
        assert (
            type(entities_or_path) == list and type(entities_or_path[0]) == list
        ), "entities should be list of list of entities"
        ...
        NumeratedRightContexts = self.get_NumeratedRightContexts(
        ...
        NumeratedUnitsAsInputVars = self.get_NumeratedUnitsAsInputVars(
            num_mentions=num_mentions
        )
        NumeratedUnitsAsInputVarsConcat = self.get_NumeratedUnitsAsInputVarsConcat(
            num_mentions=num_mentions
        )

        NumeratedUnitDerivationRules = self.get_NumeratedUnitDerivationRules(
            num_mentions=num_mentions
        )
        NumeratedRightContextMaterialisationRules = (
            self.get_NumeratedRightContextMaterialisationRules(
                right_contexts=contexts[1:]
            )
        )
        NumeratedMentionMaterialisationRules = (
            self.get_NumeratedMentionMaterialisationRules(mentions=mentions)
        )

        # flatten the list of lists
        all_entities = [entity for entity_list in entities for entity in entity_list]
        EntitiesMaterialisationRules = self.batch_get_decoding_linearization_rules(
            entities=all_entities, rm_eos=True, rm_bos=True
        )
    ...
    def get_NumeratedMentions(self, num_mentions: int):
        mentions = [
            self.NUMERATED_MENTION_TEMPLATE.format(i=i) for i in range(num_mentions)
        ]
        return " , ".join(mentions)

    def get_NumeratedRightContexts(self, num_mentions: int):
        right_contexts = [
            self.NUMERATED_RIGHT_CONTEXT_TEMPLATE.format(i=i)
    ...
    def get_NumeratedEntities(self, num_mentions: int):
        entities = [
            self.NUMERATED_ENTITY_TEMPLATE.format(i=i) for i in range(num_mentions)
        ]
    ...
    def get_NumeratedUnitDerivationRules(self, num_mentions: int):
        rules = [
            self.NUMERATED_UNIT_DERIVATION_RULE_TEMPLATE.format(i=i)
            for i in range(num_mentions)
        ]
        return self.join_statements_multi_line(statements=rules)

    def get_NumeratedRightContextMaterialisationRules(
        self, right_contexts: List[str] = None
    ):
        right_context_tokens: List[str] = [
            self.get_entity_tokens(entity=right_context, rm_eos=True)
            for right_context in right_contexts
        ]
        rules = [
            self.NUMERATED_RIGHT_CONTEXT_MATERIALISATION_RULE_TEMPLATE.format(
                i=i, tokens=right_context_tokens[i]
            )
            for i in range(len(right_contexts))
        ]
        return self.join_statements_multi_line(statements=rules)

    def get_NumeratedMentionMaterialisationRules(self, mentions: List[str] = None):
        mention_tokens: List[str] = [
            self.get_entity_tokens(entity=mention, rm_eos=True) for mention in mentions
        ]
        rules = [
            self.NUMERATED_MENTION_MATERIALISATION_RULE_TEMPLATE.format(
                i=i, tokens=mention_tokens[i]
            )
...
if __name__ == "__main__":
    grammar_builder = ED_DepMultiMentionCrtGrammarBuilder(
        tokenizer_or_path="/Users/saibo/Research/llama_hf/7B", literal=True
    )
    grammar = grammar_builder.build(
        base_grammar_name="ED-efficient",
        mentions=["mention0", "mention1", "mention2"],
        contexts=["context0", "context1", "context2", "context3"],
        entities_or_path=[["ent0", "ent1", "ent2"]],
    )
