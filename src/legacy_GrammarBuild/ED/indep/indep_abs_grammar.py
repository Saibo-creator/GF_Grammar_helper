...
class ED_IndepMinimalAbsGrammarBuilder(TemplateTokenGrammarBuilder):
    template = os.path.join(
        TEMPLATE_DIR, "ED", "InDep", "minimal", "ED-Indep-Minimal-AbsTemplate.hs"
    )
    ...
    def add_entities_derivative_rules(
        self, entities: List[str], entity_idx=None
    ) -> str:
        ...
        statement = (
            self.join_statements_multi_line(statements=entities_ids)
            + f": Entity{entity_idx};"
            if entity_idx is not None
            else self.join_statements_multi_line(statements=entities_ids)
        )
        return statement
